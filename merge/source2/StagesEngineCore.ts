...
import { StagesLogicCoordinator } from "./StagesLogicCoordinator";
import { StagesRendererCore } from "./StagesRendererCore";
import { StagesEngineObjectsManager } from "./StagesEngineObjectsManager";
import { StagesEngineEventsManager } from "./StagesEngineEventsManager";
import type {
  StageConfig,
  StageObject,
  StageEvent,
  ...
  DeviceTier,
  ViewportTransform,
} from "./StagesSystemTypes";

export class StagesEngine {
  // Single coordination point (replaces device/transform/performance)
  private logic: StagesLogicCoordinator;

  // Core modules (stable)
  private renderer: StagesRendererCore;

  // Logic modules (AI can modify these via child files)
  private objectManager: StagesEngineObjectsManager;
  private eventManager: StagesEngineEventsManager;
  ...
  constructor(config: StageConfig = {}) {
    ...
    this.logic = new StagesLogicCoordinator(config);

    // Initialize renderer with logic's performance module
    this.renderer = new StagesRendererCore(this.logic);

    // Initialize logic modules (AI modifies these)
    this.objectManager = new StagesEngineObjectsManager();
    this.eventManager = new StagesEngineEventsManager();
