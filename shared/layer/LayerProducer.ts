...
export function produceLayers(
  ...
  const out: LayerData[] = layers.map(
    (norm: LayerConfigNormalized, idx: number) => {
      ...
      const pulseRes = applyPulse(
        { scale: basic.scale },
        norm.behaviors.pulse,
        ctx.time,
      );
      const fadeRes = applyFade(
