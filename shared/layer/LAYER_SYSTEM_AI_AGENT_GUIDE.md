...
interface LayerData {
  ...
  container?: {
    // Image container settings (if layer has dimensions)
    width?: number;
    height?: number;
    fitMode: FitMode;
    alignment: Alignment;
...
```typescript
function applyPulse(prev: { scale: Vec2 }, cfg: PulseConfig, timeSeconds: number): { scale: Vec2 };
```
...
async function preloadAssets(assetPaths: string[]): Promise<Map<string, AssetMeta>> {
  ...
  await Promise.all(
    assetPaths.map(async (path, index) => {
      try {
        const img = new Image();
        img.src = path;
