...
export function fromUILayerControls(
  ...
  const opacity =
    ui.opacity !== undefined
      ? fromUIOpacity(ui.opacity, opacityOpts)
      : base.opacity;

  const uniform =
    ui.scale !== undefined
      ? fromUIScaleUniform(ui.scale, scaleOpts)
      : base.scale;

  const spin: SpinConfig = {
    enabled: ui.spin?.enabled ?? base.behaviors.spin.enabled,
    rpm:
      ui.spin?.rpm !== undefined
        ? fromUIRpm(ui.spin.rpm, rpmOpts)
        : base.behaviors.spin.rpm,
  ...
  const orbit: OrbitConfig = {
    ...
    rpm:
      ui.orbit?.rpm !== undefined
        ? fromUIRpm(ui.orbit.rpm, rpmOpts)
        : base.behaviors.orbit.rpm,
    radius:
      ui.orbit?.radius !== undefined
        ? ui.orbit.radius
        : base.behaviors.orbit.radius,
  ...
  const pulse: PulseConfig = {
    ...
    amplitude:
      ui.pulse?.amplitude !== undefined
        ? ui.pulse.amplitude
        : base.behaviors.pulse.amplitude,
    rpm:
      ui.pulse?.rpm !== undefined
        ? fromUIRpm(ui.pulse.rpm, rpmOpts)
        : base.behaviors.pulse.rpm,
  };
  ...
  const fade: FadeConfig = {
    ...
    rpm:
      ui.fade?.rpm !== undefined
        ? fromUIRpm(ui.fade.rpm, rpmOpts)
        : base.behaviors.fade.rpm,
  };
