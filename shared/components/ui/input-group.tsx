...
const InputGroupText = React.forwardRef<HTMLSpanElement, InputGroupTextProps>(
  ({ className, ...props }, ref) => {
    return (
      <span
        ref={ref}
        className={cn("flex h-9 items-center px-3 text-sm text-muted-foreground", className)}
        {...props}
      />
    );
  },
...
const InputGroupInput = React.forwardRef<HTMLInputElement, InputGroupInputProps>(
  ({ className, ...props }, ref) => {
    return <Input ref={ref} className={cn("flex-1", className)} {...props} />;
  },
);
...
const InputGroupTextarea = React.forwardRef<HTMLTextAreaElement, InputGroupTextareaProps>(
  ({ className, ...props }, ref) => {
    return <Textarea ref={ref} className={cn("flex-1", className)} {...props} />;
  },
);
