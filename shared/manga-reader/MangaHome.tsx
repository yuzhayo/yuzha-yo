...
import type { HistoryEntry, ScannedSeries } from "@shared/manga-types";
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-5 px-4 pb-8 flex-1">
        ...
        {folderState.status === "restoring" && (
          <div className="text-sm text-neutral-400">🔄 Reopening your last folder…</div>
        )}

        {folderState.status === "needs-permission" && (
          <div className="bg-amber-900/30 border border-amber-700/50 rounded-xl px-4 py-3 flex items-center justify-between gap-3">
            <p className="text-sm text-amber-200">
              Permission needed to reopen "{folderState.folderName}".
            </p>
            <button
              type="button"
              onClick={onGrantAccess}
              className="flex-shrink-0 px-3 py-1.5 rounded-lg bg-amber-700 hover:bg-amber-600 active:bg-amber-800 transition-colors text-sm font-medium"
            >
              Allow Access
            </button>
          </div>
        ...
        {history.length === 0 &&
          !(["ready", "scanning", "restoring", "needs-permission"] as string[]).includes(
            folderState.status,
          ) && (
