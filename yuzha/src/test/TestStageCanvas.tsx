 ...
 * - TestStageCanvas.tsx (this file) - Uses Canvas 2D API
...
async function mountCanvasLayers(
  ...
  layersWithProcessors: Array<{ data: TestLayerData; processors: TestLayerProcessor[] }>,
  ...
  for (const item of layersWithProcessors) {
    try {
      ...
      if (IS_DEV) {
        console.error(`[TestStageCanvas] Failed to load image for "${item.data.ImageID}"`, error);
      }
    }
  }
  ...
  const pipelineCache = createPipelineCache<TestLayerData>();

  /**
   * Renders a single layer to the canvas.
   * Handles positioning, rotation, and scaling.
   */
  const renderLayer = (layer: CanvasLayerEntry, enhancedData: TestLayerData) => {
    ...
    const position =
      enhancedData.hasOrbitalAnimation || enhancedData.hasSpinAnimation
        ...
        : roundTestStagePoint(enhancedData.position);
  ...
  const renderFrame = (timestamp: number) => {
    ctx.clearRect(0, 0, TEST_STAGE_SIZE, TEST_STAGE_SIZE);
    const frameData = new Map<string, TestLayerData>();
...
async function mountCanvasRenderer(
  ...
  if (!context) {
    throw new Error("[TestStageCanvas] Failed to acquire 2D context");
  }
  ...
  const cleanupTransform = createTestStageTransformer(canvas, container, {
    resizeDebounce: 100,
  });
...
/**
 * TestStageCanvas React Component
 *
 * LIFECYCLE:
 * 1. Component mounts
...
function TestStageCanvas({ loadPipeline = createTestStagePipeline }: TestStageCanvasProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      console.error("[TestStageCanvas] Failed to initialize Canvas stage", error);
    });
...
export default React.memo(TestStageCanvas);
